#!/usr/bin/env bash
# Install built binaries and packaged session/config files into the package tree.
set -euo pipefail
...
install -d "$DEST/usr/libexec/y5-compositor"
...
install -m755 "$SESSION/session" "$DEST/usr/libexec/y5-compositor/session"
...
install -m644 "$STAGE/templates/mx/42-logitech-hidpp.rules" \
	"$DEST/usr/lib/udev/rules.d/42-logitech-hidpp.rules"
...
install -m644 "$STAGE/templates/mx/config.example.toml" \
	"$DEST/usr/share/y5-compositor/mx-gesture-daemon/config.example.toml"

install -m644 "$ROOT/debian/README.Debian" "$DEST/usr/share/doc/y5-compositor/README.Debian"
if [ -f "$ROOT/debian/NEWS.Debian" ]; then
	install -m644 "$ROOT/debian/NEWS.Debian" "$DEST/usr/share/doc/y5-compositor/NEWS.Debian"
fi
